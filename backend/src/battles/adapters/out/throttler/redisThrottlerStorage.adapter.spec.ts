...
describe('RedisThrottlerStorageAdapter', () => {
  ...
  it('Redis eval 결과를 ThrottlerStorageRecord로 변환한다', async () => {
    ...
    const result = await storage.increment('throttler:battle:chat:battle-1:user-1', 5000, 5)
  ...
  it('limit을 초과하면 blocked 상태와 남은 block 시간을 반환한다', async () => {
    ...
    const result = await storage.increment('key', 5000, 5)
  ...
  it('Redis TTL이 비정상이면 요청 ttl을 fallback으로 사용한다', async () => {
    ...
    const result = await storage.increment('key', 1000, 5)
  ...
  it('Redis eval 실패를 RedisThrottlerStorageError로 감싼다', async () => {
    ...
    await expect(storage.increment('key', 1000, 5)).rejects.toThrow(RedisThrottlerStorageError)
  })
