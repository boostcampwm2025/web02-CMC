...
const PREFIX = 'battle_stress_mix'
...
async function prepareStressMixBattle(context, events, done) {
  try {
    const battle = await createPublicBattle('load-test-stress-mix')
...
async function runStressMixSession(context, events, done) {
  ...
  try {
    ...
    const chatLoop = runLoop({
      ...
      minIntervalMs: scaledIntervalMs('CHAT_MIN_INTERVAL_MS', context.vars.chatMinIntervalMs, 1050),
      maxIntervalMs: scaledIntervalMs('CHAT_MAX_INTERVAL_MS', context.vars.chatMaxIntervalMs, 1250),
      warmupMultiplier: 2,
      action: async () => {
        ...
        socket.emit(CLIENT_EVENTS.CHAT, {
          ...
          scope,
          team,
          text: `stress mix chat ${guest.id} ${Date.now()}`,
        })
    ...
    const attackLoop = runLoop({
      ...
      warmupMs,
      isDisconnected: () => disconnected,
      minIntervalMs: scaledIntervalMs('ATTACK_MIN_INTERVAL_MS', context.vars.attackMinIntervalMs, 7200),
      maxIntervalMs: scaledIntervalMs('ATTACK_MAX_INTERVAL_MS', context.vars.attackMaxIntervalMs, 8800),
      warmupMultiplier: 2,
      action: async () => {
        socket.emit(CLIENT_EVENTS.ATTACK, {
          battleId,
          team,
          content: `stress mix attack ${guest.id} ${Date.now()}`,
        })
...
async function runVoteLoop({ context, events, socket, battleId, team, vuId, userId, startedAt, deadline, warmupMs, isDisconnected }) {
  const minIntervalMs = scaledIntervalMs('VOTE_MIN_INTERVAL_MS', context.vars.voteMinIntervalMs, 1200)
  const maxIntervalMs = scaledIntervalMs('VOTE_MAX_INTERVAL_MS', context.vars.voteMaxIntervalMs, 1450)
  let voteIndex = vuId
  ...
  for (const team of ['A', 'B']) {
    ...
    for (let i = 0; i < seedPerTeam; i += 1) {
      ...
      const created = await postJson(`${getApiUrl()}/dev/battles/${battleId}/discussion`, {
        ...
        content: `stress mix attack seed ${team}-${i + 1}`,
      })
...
function scaledIntervalMs(envName, contextValue, fallback) {
  if (process.env[envName]) return asNumber(process.env[envName], fallback)
  const scale = Math.max(asNumber(process.env.STRESS_SCALE, 1), 0.1)
  return Math.max(Math.round(asNumber(contextValue, fallback) / scale), 1)
}
